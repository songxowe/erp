...
<html>
  <head>
    ...
    function check() {
		$.post("<%=request.getContextPath() %>/scellController_check2.do", {
			"storeId" : $("#storeId").val()
		}, function(data) {
			if (data == 1) {
				alert("审核成功");
				window.location="scellcheck.jsp";
			} else {
				alert("审核失败");
				history.go(-1);
			}
  ...
  <form action="<%=request.getContextPath() %>/scellController_check2.do" id="scelladd" method="post">
  	...
  	<div align="right" style="width: 1062px">
  	<a class="easyui-linkbutton" data-options="iconCls:'icon-add'" id="sub" href="#" onClick="check()">复核通过</a>
  	<a href="scellcheck.jsp" class="easyui-linkbutton" data-options="plain:false,iconCls:'icon-back'">返回</a>
  	</div>
