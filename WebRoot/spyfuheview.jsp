...
<body bgcolor="#ccccff" >
	 <form action="<%=request.getContextPath() %>/fzjspayController_modifyCheckTag.do" id="b" method="post">
  	<input type="hidden" name="payId" value="${spa.payId}" />
  	<div align="right" style="width: 990px">
  	<a class="easyui-linkbutton" data-options="iconCls:'icon-add'" id="sub" onClick="queren()">通过</a>
  	<a class="easyui-linkbutton" data-options="iconCls:'icon-no'" href="fuheerror.jsp">不通过</a>
	...
	<label>入库单编号：<input readonly="readonly" name="gatherId" value="${spa.payId}" style="border: hidden;" /></label>
	</td>	
	</tr>
	<tr>
	<td colspan="3" style="border: hidden;">
	...
	<label>入库详细理由:<input readonly="readonly" name="reasonexact" value="${spa.reasonexact}" style="border: hidden;" /></label>	
      ...
      <tr style="text-align: center;border: hidden">
    	...
    	<td bgcolor="#ffcc33"><input type="text" readonly="readonly" name="nowAmountb" style="border: hidden;background-color:#ffcc33;text-align: center;"  value="${spa.nowAmountb}"/></td>
	...
	<label>已入库总成本:<input readonly="readonly" name="realSalAllb" value="${spa.realSalAllb }" style="border: hidden;" /></label>	
		...
		<tr>
    <td colspan="3" style="border: hidden;"><label>复核人：<input  name="register" readonly="readonly" style=" border-top: thin;border-left:thin;border-right:thin;border-bottom-width:1  ;border-color: #000000; width: 287px" value="${spa.register}"/></label></td>
	<td colspan="3" style="border: hidden;">
	复核时间:<SPAN id="registerTime"></SPAN>
	...
	function closeWindowOfMenuView() {
		$("#editsg3").window("close", true);
	}
	
	function queren(){
	  
	  var i=window.confirm("是否确认通过复核?");
	   if(i){
