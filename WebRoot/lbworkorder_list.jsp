...
<body bgcolor="#ebf3fb" oncontextmenu="event.returnValue=true" 
	...
	<table class="TABLE_STYLE2" width="100%">
		<tr height=20 class="TR_STYLE1">
			<td class="TD_STYLE3">当前等待登记的生产派工单总数：<div style="display:inline" id="myid"><c:if test="${ANUM==null}">0</c:if>${ANUM}</div>例<c:remove var="ANUM"/></td>
		</tr>
	</table>
<body>
		...
		$("#workorderDataGrid").datagrid({
			...
			columns:[[
			{
				...
				formatter:function(value,row,index){
					...
					url="lbManufactureController_detail.do?manufactureId="+temp;
						return "<a href="+url+">登记</a>";
				}
			}
			]],
			pagination:"true",
