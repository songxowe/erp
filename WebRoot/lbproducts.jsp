...
<script type="text/javascript">
	function check(id,mid){
		 $.get("lbManufactureController_ajax.do?id="+id+"&mid="+mid,function(data){
        if(data==null||data==""){
        	location.href="lbManufactureController_material.do?id="+id+"&mid="+mid;
        }else{ 
        	alert("上道工序未完成");
        	return;
        } 
...
<body bgcolor="#ebf3fb" oncontextmenu="event.returnValue=true"  style="filter:progid:DXImageTransform.microsoft.gradient(gradienttype=0,startColorStr=#83c6e0,endColorStr=#ffffff">
 ...
 <td  class="TD_HANDBOOK_STYLE1">您正在做的业务是：生产管理--内部生产管理--生产登记（<a href="javascript:winopens('../../main/responsiblePerson.htm')">
责任人控制</a>）</td>
 </tr>
 </table>
...
<c:if test="${list.procedureFinishTag=='G004-0'}">
	<a href="#" onclick="check(${manufacture.id},${list.id})">登记</a>
</c:if>
<c:if test="${list.procedureFinishTag=='G004-2'}">
	<!-- <a href="register_procedure_details.jsp?manufacture_ID=03050020161027100001&&procedure_name=%E7%BB%84%E8%A3%85">登记</a> -->
	<span style="color:red">等待审核</span>
</c:if>
<c:if test="${list.procedureFinishTag=='G004-3'}">
	<span>完成</span>
</c:if>
</td>
<td bordercolor=#DEDBD6 align=left class="TD_STYLE2">
<c:if test="${list.procedureFinishTag=='G004-3'&&list.procedureTransferTag=='G005-0'}">
  <a href="#" onclick="gonew()">交接登记</a>
</c:if>
<c:if test="${list.procedureTransferTag=='G005-1'||list.procedureTransferTag=='G005-2'}">
  完成
</c:if>
</td>
</tr>
...
<form id="myform" action="lbManufactureController_changecheck.do" method="post">
<input type="hidden" name="id" value="${manufacture.id}">
<input type="hidden" id="mamount" value="${manufacture.amount}">
<c:forEach items="${mlist}" var="list">
	<c:if test="${list.procedureFinishTag=='G004-3'&&list.procedureTransferTag=='G005-0'}">
